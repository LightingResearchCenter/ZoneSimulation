...
ZT = readtable('\\root\public\snydej7\zone size project\simulation list.xlsx','Sheet','groups of 4');
ZT(isnan(ZT.Zone),:) = [];
...
for iZ = 1:height(ZT)
    ...
    thisPosition = [ZT.Troffer_3_x(iZ), ZT.Troffer_3_y(iZ)];
    if ~any(isnan(thisPosition))
        Sim.Zones(iZ,1).Luminaires(3,1) = findobj(Sim.Luminaires,'Position',thisPosition);
    end
    
    thisPosition = [ZT.Troffer_4_x(iZ), ZT.Troffer_4_y(iZ)];
    if ~any(isnan(thisPosition))
        Sim.Zones(iZ,1).Luminaires(4,1) = findobj(Sim.Luminaires,'Position',thisPosition);
    end
end

% Save, clear, reload
save('Groups4.mat','Sim')
clear
load('Groups4.mat')
